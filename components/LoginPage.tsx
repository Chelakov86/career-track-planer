...
export const LoginPage: React.FC<LoginPageProps> = ({ language, setLanguage }) => {
  const { login, isLoading, isRealAuthAvailable } = useAuth();
  const t = TRANSLATIONS[language];
  const googleButtonRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (isRealAuthAvailable && googleButtonRef.current && window.google) {
      window.google.accounts.id.renderButton(
        googleButtonRef.current,
        { theme: 'outline', size: 'large', width: 350, text: 'signin_with' } // Customization
      );
    }
