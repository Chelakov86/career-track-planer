...
export const JobCard: React.FC<JobCardProps> = React.memo(({
    ...
    return (
        <div
            className={`bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm transition-all ${accentBorder} ${
                isGhost ? 'shadow-2xl ring-2 ring-primary rotate-3 z-50 opacity-90' :
        ...
        >
            ...
            {!isGhost && onView && (
                <button
                    ...
                    className="w-full py-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium hover:bg-primary hover:text-white transition-all"
                >
                    {t.board.viewDetails}
                </button>
            )}
